function Install-VSCodeAndGit {
  ...
  .NOTES
    ...
    ChangeLog 
      ...
      v1.4.0 25-May-2023 Added new configs to settings.json to change the highlight colors, auto install PS ext 
  #>
  [cmdletbinding()]
  Param (
    [string]$GitHubRepoURL = (Read-Host -Prompt "Type the URL to clone the GitHub Repository, you will find it in the green Code button in Github"),
  ...
  do {
    ...
    $CodePath = $CodeProc | Select-Object -Property Path -Unique
  } until ($CodeProc.Count -ge 1)
  Write-Verbose "Just before installing the PS Extension"
  Write-Debug "Just before installing the PS Extension"  
  Invoke-Expression -Command "$CodePath --install-extension ms-vscode.powershell"
  do {
    $InstalledPSExt = Invoke-Expression -Command "$CodePath --list-extensions"
  } until ($InstalledPSExt -contains 'ms-vscode.powershell')
